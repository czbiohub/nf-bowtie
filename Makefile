...
demo1:
	nextflow run main.nf -profile czbiohub_aws --skip_fastqc --reference_type single_file --reads 's3://kalani-bucket/FLASH/fastq/**{R1,R2}_001.fastq' --reference 's3://kalani-bucket/FLASH/amrFLASH_all_98percent_1433.fasta' --outdir './results_flash_demo'

demo2:
	nextflow run main.nf -profile czbiohub_aws --skip_fastqc --reference_type single_file --reads 's3://kalani-bucket/nextflow_demo/fastq/**_{R1,R2}_cdh_lzw_trim30_PF.fastq.gz' --reference 's3://kalani-bucket/nextflow_demo/references/contig.fasta' --outdir './results_cms_demo'
